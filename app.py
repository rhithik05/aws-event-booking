...
def home():
    if 'user_id' in session:
        print(f"User ID from session: {session['user_id']}")  # Debug print statement
        return render_template("index.html")
    else:
        print("No user_id found in session.")  # Debugging: No session found
        return redirect(url_for('login'))  # Redirect to login page if not logged in

@app.route("/index")
def index_redirect():
    return redirect(url_for('home'))  # Redirect /index to /home
...
@login_required  # Ensuring only logged-in users can book an event
def book_event():
    if request.method == 'POST':
        ...
        try:
            connection = get_db_connection()
            with connection.cursor() as cursor:
                # Convert event_date to MySQL DATE format (YYYY-MM-DD)
                event_date = datetime.strptime(event_date, "%Y-%m-%d").date()
                
                # SQL query to insert data into the event_bookings table
                sql = """INSERT INTO event_bookings 
                         (user_id, full_name, email, phone, event_type, event_date, guests, special_requests) 
                         VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
                
                values = (session['user_id'], full_name, email, phone, event_type, event_date, guests, special_requests)
...
def history():
    ...
    # Fetch only bookings related to the logged-in user
    cursor.execute('''
        SELECT id, full_name, email, phone, event_type, event_date, guests, special_requests
        FROM event_bookings
        WHERE user_id = %s
        ORDER BY event_date DESC
    ''', (session['user_id'],))  # Filter by user_id
